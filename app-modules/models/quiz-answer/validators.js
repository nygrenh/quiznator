...
module.exports = schema => {
  ...
  function validateQuizExpiration(quiz) {
    if(quiz.expiresAt && +new Date(quiz.expiresAt) - +new Date() <= 0) {
      return Promise.reject(new errors.InvalidRequestError(`Couldn't create an answer because quiz has expired`));
    } else {
      return Promise.resolve();
    }
  }
  ...
  schema.pre('save', function(next) {
    ...
    errors.withExistsOrError(new errors.NotFoundError(`Couldn't find quiz with id ${this.quizId}`))
      (mongoose.models.Quiz.findOne({ _id: this.quizId }))
        .then(quiz => {
          return Promise.all([validateAnswerData.call(this, quiz), validateQuizExpiration.call(this, quiz)]);
        })
        .then(() => next())
        .catch(err => next(err));
  });
