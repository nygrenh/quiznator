...
function validateOpenData(quiz, data) {
  ...
  const answer = data.replace(nonPrintingCharRegex, "")
  ...
  if (meta.regex) {
    try {
      ...
      correct = !!re.exec(answer.trim().toLowerCase())
    } catch (err) {
      // errored, already false
    }
  } else {
    correct = answer.trim().toLowerCase() === rightAnswer.trim().toLowerCase()
  }
...
function validateMultipleOpenData(quiz, data) {
  ...
  items.map(item => {
    ...
    const answer = data[item.id].replace(nonPrintingCharRegex, "")
